...
func _on_CheckAnswerB_pressed():
	if ($Excercise/Button.pressed && $Excercise/Button5.pressed && 
	$Excercise/Button8.pressed):
		if ($Excercise/Button2.pressed || $Excercise/Button3.pressed || 
		$Excercise/Button4.pressed || $Excercise/Button6.pressed|| $Excercise/Button7.pressed):
			UINode.ShowPopUp("This is not correct. Remember to read the theory carefully. You probably selected too much options.")
		else:
			UINode.ShowPopUp("Congratulations! You finished the challenges!")
			$Theory.visible = true
			$Excercise.visible = false
			self.visible = false
			Scenarios.FinishedScenario(5)
	else:
